...
// server/freefire.ts
var freefire_exports = {};
__export(freefire_exports, {
  decryptCredentials: () => decryptCredentials,
  encryptCredentials: () => encryptCredentials,
  fetchAccountInfo: () => fetchAccountInfo,
  parseAccountInput: () => parseAccountInput,
  parseGarenaJson: () => parseGarenaJson,
  parseUidPassword: () => parseUidPassword
});
function parseUidPassword(input) {
...
function parseGarenaJson(input) {
  try {
...
async function fetchAccountInfo(credentials) {
  ...
  try {
    ...
    if (!data.id || !data.nickname) {
      throw new Error("Resposta da API inv\xE1lida");
    }
    return {
      id: data.id,
...
var FF_API_BASE;
var init_freefire = __esm({
  "server/freefire.ts"() {
    "use strict";
    init_crypto();
    FF_API_BASE = "https://main.ffapi.cloud/api/freefire";
  }
});
...
var appRouter = router({
  ...
  account: router({
     ...
     */
    query: publicProcedure.input((val) => {
      if (typeof val !== "object" || val === null || !("input" in val)) {
        throw new Error("Input inv\xE1lido");
      }
      const input = val.input;
      if (typeof input !== "string" || input.trim().length === 0) {
        throw new Error("Entrada de dados inv\xE1lida");
      }
      return { input };
    }).mutation(async ({ input }) => {
      const { parseAccountInput: parseAccountInput2, fetchAccountInfo: fetchAccountInfo2 } = await Promise.resolve().then(() => (init_freefire(), freefire_exports));
      const credentials = parseAccountInput2(input.input);
      if (!credentials) {
        throw new Error("Formato de entrada inv\xE1lido. Use UID:PASSWORD ou o formato JSON da Garena.");
      }
      const accountInfo = await fetchAccountInfo2(credentials);
      return {
        success: true,
        data: accountInfo
      };
